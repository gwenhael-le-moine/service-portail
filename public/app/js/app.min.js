...
CCNUM:{action:function(){f.modification||(f.tree={configurable:!1,filter:e,aside_template:"app/views/aside_CCNUM.html?v\x3d"+l,tiles:g.pad_tiles_tree([b].concat(m.query().map(function(b,d){b.taxonomy="ccn";b.index=d+1;_(b).has("leaves")&&(b.action=function(){f.tree={configurable:!1,filter:e,aside_template:"app/views/aside_CCNUM_archives.html?v\x3d"+l,tiles:[c(a)].concat(b.leaves.map(function(a,b){a.taxonomy="ccn";a.index=b+1;return a}))};f.parent=b});return b})))},f.parent=a)}},GAR:{action:function(){f.modification||
k.ressources().then(function(c){f.tree={configurable:!1,filter:e,aside_template:"app/views/aside_RN.html?v\x3d"+l,tiles:g.pad_tiles_tree([b].concat(c.map(function(a,b){a.taxonomy="rn";a.index=b+1;a.icon=r+"/app/node_modules/laclasse-common-client/images/"+("MANUEL"===a.type?"05_validationcompetences.svg":"AUTRE"===a.type?"07_blogs.svg":"08_ressources.svg");a.color=h[b%16].color;a.action=function(){g.log_and_open_link("GAR",a.url)};return a})))};f.parent=a})}},TROMBI:{action:function(){f.modification||
...
"GRP"===a.type||_(f.filter_criteria).has("show_groupes_libres")&&f.filter_criteria.show_groupes_libres&&"GPL"===a.type)&&(!_(a).has("name")||_(f.filter_criteria.text).isEmpty()||a.name.toUpperCase().includes(f.filter_criteria.text.toUpperCase())||!_(a).has("structure")||_(f.filter_criteria.text).isEmpty()||a.structure.name.toUpperCase().includes(f.filter_criteria.text.toUpperCase()))}},aside_template:"app/views/aside_TROMBI_regroupements.html?v\x3d"+l,tiles:g.pad_tiles_tree([b].concat(e.map(function(b,
...
aside_template:"app/views/aside_TROMBI_people.html?v\x3d"+l,tiles:g.pad_tiles_tree([c(a)].concat(b.data.map(function(a,b){a.taxonomy="eleve";a.index=b+1;a.color="jaune";a.color+=0===b%2?"":"-moins";a.avatar=(_(a.avatar.match(/^(user|http)/)).isNull()?u+"/":"")+a.avatar;return a})))};f.parent=a})};return b})))};f.parent=a}))}}};a.configure=!1;a.toggle_configure=function(){a.configure&&a.update();f.tree.tiles.forEach(function(b){b.configure=b===a?!b.configure:!1})};a.update=function(){p.update(a)};
a.remove=function(){p.delete(a).$promise.then(function(a){q()})};!_(n[a.application_id]).isUndefined()&&_(n[a.application_id]).has("action")?a.action=n[a.application_id].action:a.action=function(){f.modification||("EXTERNAL"===a.type||_(a.application_id).isNull()||"PRONOTE"===a.application_id?g.log_and_open_link("PRONOTE"===a.application_id?"PRONOTE":"EXTERNAL",a.url):d.go("app",{appid:a.application_id}))};!_(n[a.application_id]).isUndefined()&&_(n[a.application_id]).has("modify")&&(a=n[a.application_id].modify(a));
return a},q=function(){k.tiles().then(function(a){a.forEach(function(a){a.taxonomy="app"});a=_(a).select(function(a){return(7!=moment().month()||a.summer||!_(f.user.profiles).isEmpty()&&!_(["ELV","TUT"]).includes(f.user.active_profile().type))&&(!f.user.profiles||!f.user.active_profile()||f.user.is_admin()||!_(a.hidden).includes(f.user.active_profile().type))&&("MAIL"===a.application_id?_.chain(f.user.emails).pluck("type").includes("Ent").value():!0)}).map(c);a=g.fill_empty_tiles(a);a=_(a).sortBy(function(a){return a.index});
a=g.pad_tiles_tree(a);f.tiles={configurable:!0,aside_template:"app/views/aside_news.html?v\x3d"+l,tiles:a};b.action()})};f.modification=!1;f.edit_tiles=function(){f.modification=!0};f.exit_tiles_edition=function(){f.modification=!1;f.tree.tiles.forEach(function(a){_(a).has("configure")&&(a.configure=!1)});q()};f.sortable_options={accept:function(a,b){return!0},longTouch:!0,dragEnd:function(a){_(f.tree.tiles).map(function(a,b){_(a).has("id")&&(a.index=b,a.update())})},containment:".damier",containerPositioning:"relative",
additionalPlaceholderClass:"col-xs-6 col-sm-4 col-md-3 col-lg-3 petite case",clone:!1,allowDuplicates:!1};f.add_tile=function(a){v.add_tiles(a,function(b){var c=_.chain(a).reject(function(a){return _(a).has("taxonomy")}).pluck("index").sort().value();_(b).each(function(b){b.structure_id=f.user.active_profile().structure_id;if(1>c.length){var e=a.length;a.push({index:e})}else e=c.shift();console.log(c);console.log(e);b.index=e;p.save({},b).$promise.then(function(a){q()})})},function(){})};q()})}}],
