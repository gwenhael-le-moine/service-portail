...
!0;a.defaults.withCredentials=!0;a.interceptors.push(["$q","$window",function(a,c){return{responseError:function(a){401===a.status&&(c.location.href="/sso/login?ticket\x3dfalse\x26service\x3d"+encodeURIComponent(c.location.href));return a}}}])}]).run(["$rootScope","log",function(a,d){a.modification=!1;a.$on("$stateChangeSuccess",function(a,b,g,e,k){d.add("app"==b.name?g.appid:"PORTAIL",null,null)})}]);
...
var e=b.user.new_avatar.image;var d=0<=e.split(",")[0].indexOf("base64")?atob(e.split(",")[1]):unescape(e.split(",")[1]);e=e.split(",")[0].split(":")[1].split(";")[0];for(var g=new Uint8Array(d.length),k=0;k<d.length;k++)g[k]=d.charCodeAt(k);d=new Blob([g],{type:e});a.blob=d};c.src=b.user.new_avatar.image;b.processing=!1})};b.onChange=function(a){e(a.target.files[0])};var k=function(){b.user.new_avatar={image:null,blob:null,width:0,height:0}};b.upload_avatar=function(){b.user.$upload_avatar().then(function(a){k()})};
...
angular.module("portailApp").component("news",{bindings:{edition:"\x3c"},controller:["$sce","Popups","$http","$q","URL_ENT","RANDOM_IMAGES","currentUser",function(a,d,c,b,g,e,k){var h=this;h.newsfeed=[];h.retrieve_news=function(d){var k=moment().subtract(1,"months").toDate().toISOString();c.get(g+"/api/news",{params:{user_id:h.user.id,"pubDate\x3e":k}}).then(function(e){h.newsfeed=_(e.data).map(function(b,c){b.trusted_content=a.trustAsHtml(b.description);b.no_image=_(b.image).isNull();b.pubDate=moment(new Date(b.pubDate)).toDate();
...
angular.module("portailApp").component("portail",{controller:["$sce","$state","$uibModal","$q","CASES","COULEURS","currentUser","Utils","CCN","Tiles","APP_PATH","CACHE_BUSTER","User","Annuaire","URL_ENT","Popups",function(a,d,c,b,g,e,k,h,m,p,q,l,r,t,v,w){var f=this;f.$onInit=function(){k.get(!0).then(function(a){f.user=a;f.prefix=q;f.COULEURS=e;f.CACHE_BUSTER=l;f.get_tile_template=function(a){return{app:"app/views/tile_app.html?v\x3d"+l,back:"app/views/tile_app.html?v\x3d"+l,regroupement:"app/views/tile_regroupement.html?v\x3d"+
l,eleve:"app/views/tile_eleve.html?v\x3d"+l,rn:"app/views/tile_rn.html?v\x3d"+l,ccn:"app/views/tile_ccn.html?v\x3d"+l}[a]};f.filter_criteria={};var c={index:0,taxonomy:"back",name:"\u2190 Retour",description:"Retour",color:"gris3",action:function(){f.tree=f.tiles;f.parent=null}},r=function(a){var b=function(a){var b=angular.copy(c);b.action=a.action;return b},e=function(){return function(a){return!0}},n={MAIL:{modify:function(a){k.recent_mail().then(function(b){console.log(b.data);a.recent_mail=b.data});
return a}},CCNUM:{action:function(){f.modification||(f.tree={configurable:!1,filter:e,aside_template:"app/views/aside_CCNUM.html?v\x3d"+l,tiles:h.pad_tiles_tree([c].concat(m.query().map(function(c,d){c.taxonomy="ccn";c.index=d+1;_(c).has("leaves")&&(c.action=function(){f.tree={configurable:!1,filter:e,aside_template:"app/views/aside_CCNUM_archives.html?v\x3d"+l,tiles:[b(a)].concat(c.leaves.map(function(a,b){a.taxonomy="ccn";a.index=b+1;return a}))};f.parent=c});return c})))},f.parent=a)}},GAR:{action:function(){f.modification||
k.ressources().then(function(b){f.tree={configurable:!1,filter:e,aside_template:"app/views/aside_RN.html?v\x3d"+l,tiles:h.pad_tiles_tree([c].concat(b.map(function(a,b){a.taxonomy="rn";a.index=b+1;a.icon=q+"/app/node_modules/laclasse-common-client/images/"+("MANUEL"===a.type?"05_validationcompetences.svg":"AUTRE"===a.type?"07_blogs.svg":"08_ressources.svg");a.color=g[b%16].color;a.action=function(){h.log_and_open_link("GAR",a.url)};return a})))};f.parent=a})}},TROMBI:{action:function(){f.modification||
(f.filter_criteria={show_classes:!0,show_groupes_eleves:!0,show_groupes_libres:!0,text:""},f.get_structure=function(a){return t.get_structure(a).then(function(a){return a.data})},k.groups().then(function(e){f.tree={configurable:!1,filter:function(){return function(a){return"back"===a.taxonomy||("regroupement"!==a.taxonomy||_(f.filter_criteria).has("show_classes")&&f.filter_criteria.show_classes&&"CLS"===a.type||_(f.filter_criteria).has("show_groupes_eleves")&&f.filter_criteria.show_groupes_eleves&&
"GRP"===a.type||_(f.filter_criteria).has("show_groupes_libres")&&f.filter_criteria.show_groupes_libres&&"GPL"===a.type)&&(!_(a).has("name")||_(f.filter_criteria.text).isEmpty()||a.name.toUpperCase().includes(f.filter_criteria.text.toUpperCase())||!_(a).has("structure")||_(f.filter_criteria.text).isEmpty()||a.structure.name.toUpperCase().includes(f.filter_criteria.text.toUpperCase()))}},aside_template:"app/views/aside_TROMBI_regroupements.html?v\x3d"+l,tiles:h.pad_tiles_tree([c].concat(e.map(function(c,
e){c.taxonomy="regroupement";c.index=e+1;switch(c.type){case "CLS":c.color="vert";break;case "GRP":c.color="bleu";break;default:c.color="jaune"}c.color+=0===e%2?"":"-moins";c.action=function(){f.filter_criteria.text="";t.get_users(_(c.users).pluck("user_id")).then(function(c){f.tree={configurable:!1,filter:function(){return function(a){return"eleve"!==a.taxonomy||_(f.filter_criteria.text).isEmpty()||a.lastname.toUpperCase().includes(f.filter_criteria.text.toUpperCase())||a.firstname.toUpperCase().includes(f.filter_criteria.text.toUpperCase())}},
aside_template:"app/views/aside_TROMBI_people.html?v\x3d"+l,tiles:h.pad_tiles_tree([b(a)].concat(c.data.map(function(a,b){a.taxonomy="eleve";a.index=b+1;a.color="jaune";a.color+=0===b%2?"":"-moins";a.avatar=(_(a.avatar.match(/^(user|http)/)).isNull()?v+"/":"")+a.avatar;return a})))};f.parent=a})};return c})))};f.parent=a}))}}};a.configure=!1;a.toggle_configure=function(){f.tree.tiles.forEach(function(b){b.configure=b.index===a.index?!b.configure:!1})};a.dirty={};a.is_dirty=function(b){a.dirty[b]=
!0};a.to_delete=!1;a.remove=function(){a.to_delete=!a.to_delete;a.dirty=a.dirty||a.to_delete;a.configure=!1};!_(n[a.application_id]).isUndefined()&&_(n[a.application_id]).has("action")?a.action=n[a.application_id].action:a.action=function(){f.modification||("EXTERNAL"===a.type||_(a.application_id).isNull()||"PRONOTE"===a.application_id?h.log_and_open_link("PRONOTE"===a.application_id?"PRONOTE":"EXTERNAL",a.url):d.go("app",{appid:a.application_id}))};!_(n[a.application_id]).isUndefined()&&_(n[a.application_id]).has("modify")&&
(a=n[a.application_id].modify(a));return a},u=function(){k.tiles().then(function(a){a.forEach(function(a){a.taxonomy="app"});a=_(a).select(function(a){return(7!=moment().month()||a.summer||!_(f.user.profiles).isEmpty()&&!_(["ELV","TUT"]).includes(f.user.active_profile().type))&&(!f.user.profiles||!f.user.active_profile()||f.user.is_admin()||!_(a.hidden).includes(f.user.active_profile().type))&&("MAIL"===a.application_id?_.chain(f.user.emails).pluck("type").includes("Ent").value():!0)}).map(r);a=h.fill_empty_tiles(a);
a=_(a).sortBy(function(a){return a.index});a=h.pad_tiles_tree(a);f.tiles={configurable:!0,aside_template:"app/views/aside_news.html?v\x3d"+l,tiles:a};c.action()})};f.modification=!1;f.edit_tiles=function(){f.modification=!0};f.exit_tiles_edition=function(){f.modification=!1;u()};a=function(a){_(f.tree.tiles).each(function(a,b){a.index=b;_(a).has("dirty")||(a.dirty={});a.dirty.index=!0})};f.sortable_options={accept:function(a,b){return!0},longTouch:!0,itemMoved:a,orderChanged:a,containment:".damier",
containerPositioning:"relative",additionalPlaceholderClass:"col-xs-6 col-sm-4 col-md-3 col-lg-3 petite case",clone:!1,allowDuplicates:!1};f.add_tile=function(a){w.add_tiles(a,function(c){b.all(_(c).map(function(b){var c=_(a).findIndex(function(a){return!_(a).has("taxonomy")});-1===c&&(c=a.length,a.push({index:c}));a[c]=r(b);a[c].index=c;_(b).has("id")||(a[c].to_create=!0)}))},function(){})};f.save_tiles_edition=function(a){a=[];a.concat(_.chain(f.tree.tiles).where({to_delete:!0}).map(function(a){switch(a.taxonomy){case "app":return p.delete({id:a.id}).$promise;
default:return console.log(a),null}}));a.concat(_.chain(f.tree.tiles).select(function(a){return _(a).has("id")&&!_(a).has("to_create")&&_(a).has("dirty")&&!_(a.dirty).isEmpty()}).map(function(a){switch(a.taxonomy){case "app":var b={};_.chain(a.dirty).keys().each(function(c){b[c]=a[c]});return p.update({id:a.id},b);default:return console.log(a),null}}));a.concat(_.chain(f.tree.tiles).where({to_create:!0}).map(function(a){switch(a.taxonomy){case "app":return a.structure_id=f.user.active_profile().structure_id,
p.save({},a).$promise;default:return console.log(a),null}}));b.all(a).then(function(a){f.tree.tiles=h.fill_empty_tiles(_(f.tree.tiles).reject(function(a){return a.to_delete}))});f.modification=!1;f.tree.tiles.forEach(function(a){_(a).has("configure")&&(a.configure=!1)})};u()})}}],template:'\n\x3cdiv class\x3d"row portail"\n     ng:if\x3d"$ctrl.user"\x3e\n    \x3cdiv class\x3d"col-xs-12 col-sm-12 col-md-4 col-lg-4 aside"\x3e\n        \x3chelp-icon class\x3d"btn-group hidden-xs help-icon"\n                   user\x3d"$ctrl.user"\x3e\x3c/help-icon\x3e\n\n        \x3clogo class\x3d"col-xs-1 col-sm-1 col-md-6 col-lg-6 logolaclasse gris4"\n              user\x3d"$ctrl.user"\x3e\x3c/logo\x3e\n\n        \x3cuser-tile user\x3d"$ctrl.user"\x3e\x3c/user-tile\x3e\n\n        \x3cdiv class\x3d"col-xs-12 col-sm-12 col-md-12 col-lg-12 hidden-xs hidden-sm aside-bottom"\n             ng:include\x3d"$ctrl.user.edit_profile ? \'app/views/aside_news.html?v\x3d\' + $ctrl.CACHE_BUSTER : $ctrl.tree.aside_template"\x3e\x3c/div\x3e\n    \x3c/div\x3e\n\n    \x3cdiv class\x3d"col-xs-12 col-sm-12 col-md-8 col-lg-8"\x3e\n        \x3cdiv class\x3d"row user-profil"\n             ng:if\x3d"$ctrl.user.edit_profile"\x3e\n            \x3cuser-profile user\x3d"$ctrl.user"\x3e\x3c/user-profile\x3e\n        \x3c/div\x3e\n\n        \x3cdiv class\x3d"row damier gris4"\n             ng:class\x3d"{\'modification\': $ctrl.modification}"\n             ng:if\x3d"!$ctrl.user.edit_profile"\x3e\n\n            \x3cul data-as-sortable\x3d"$ctrl.sortable_options"\n                data-is-disabled\x3d"!$ctrl.modification"\n                ng:model\x3d"$ctrl.tree.tiles"\x3e\n\n                \x3cli ng:repeat\x3d"tile in $ctrl.tree.tiles | filter:$ctrl.tree.filter() | orderBy:\'index\'"\n                    class\x3d"col-xs-6 col-sm-4 col-md-3 col-lg-3 petite case animate scale-fade-in {{tile.color}}"\n                    data-as-sortable-item\n                    ng:class\x3d"{ \'empty hidden-xs\': !tile.taxonomy }"\x3e\n                    \x3cdiv ng:include\x3d"$ctrl.get_tile_template( tile.taxonomy )"\x3e\x3c/div\x3e\n                \x3c/li\x3e\n            \x3c/ul\x3e\n\n            \x3c!-- Mode normal --\x3e\n            \x3cspan class\x3d"hidden-xs hidden-sm floating-button toggle big off blanc"\n                  ng:if\x3d"$ctrl.tree.configurable \x26\x26 $ctrl.user.is_admin() \x26\x26 !$ctrl.modification"\n                  ng:click\x3d"$ctrl.edit_tiles()"\x3e\x3c/span\x3e\n\n            \x3c!-- Mode modification --\x3e\n            \x3cspan class\x3d"hidden-xs hidden-sm floating-button toggle big on gris4"\n                  ng:if\x3d"$ctrl.modification"\x3e\x3c/span\x3e\n            \x3cspan class\x3d"floating-button small cancel gris3"\n                  ng:if\x3d"$ctrl.modification"\n                  ng:click\x3d"$ctrl.exit_tiles_edition()"\x3e\x3c/span\x3e\n            \x3cspan class\x3d"floating-button small save gris1"\n                  ng:if\x3d"$ctrl.modification"\n                  ng:click\x3d"$ctrl.save_tiles_edition()"\x3e\x3c/span\x3e\n\n            \x3cspan class\x3d"floating-button small action1 add-app gris1"\n                  ng:if\x3d"$ctrl.modification"\n                  ng:click\x3d"$ctrl.add_tile( $ctrl.tree.tiles )"\x3e\x3c/span\x3e\n        \x3c/div\x3e\n\n    \x3c/div\x3e\n\x3c/div\x3e\n'});
...
angular.module("portailApp").service("currentUser",["$http","$resource","$q","URL_ENT","User","Tiles","Annuaire",function(a,d,c,b,g,e,k){var h=this,m=void 0;h.get=function(c){if(_(m).isUndefined()||c)m=a.get(b+"/api/users/current").then(function(a){return new g(a.data)});return m};h.activate_profile=function(c){return h.get(!1).then(function(d){return a({method:"PUT",url:b+"/api/users/"+d.id+"/profiles/"+c,data:{active:!0}})})};h.ressources=function(){return h.get(!1).then(function(a){return k.get_structure_resources(a.active_profile().structure_id).then(function(a){return a.data})})};
h.tiles=function(){return h.get(!1).then(function(a){return _(a.profiles).isEmpty()?k.query_applications().then(function(a){a=_.chain(a).select(function(a){return _(["MAIL","DOC"]).includes(a.application_id)}).map(function(a){a.summer=!0;return a}).value();return c.resolve(a)}):e.query({structure_id:a.active_profile().structure_id}).$promise})};h.groups=function(){return h.get(!1).then(function(d){var e=_.chain(d.profiles).pluck("structure_id").uniq().value(),g=k.get_structures(e),h=_(d.groups).pluck("group_id");
h=_(h).isEmpty()?c.resolve([]):a.get(b+"/api/groups/",{params:{"id[]":h}}).then(function(a){return a.data});var m=c.resolve([]);0<_.chain(d.profiles).pluck("type").intersection(["DIR","ADM","CPE"]).value().length&&(m=a.get(b+"/api/groups/",{params:{"structure_id[]":e}}).then(function(a){return a.data}));return c.all([h,m,g]).then(function(a){var b=a.pop().data;return _.chain(a).flatten().uniq("id").map(function(a){a.structure=_(b).findWhere({id:a.structure_id});return a}).value()})})};h.recent_mail=
function(){return h.get(!1).then(function(c){return a.get(b+"/api/users/"+c.id+"/imapcheck")})}}]);
angular.module("portailApp").service("log",["$http","$state","URL_ENT","APP_PATH","currentUser",function(a,d,c,b,g){this.add=function(e,k,h){g.get(!1).then(function(g){a.post(c+"/api/logs",{application_id:e,user_id:g.id,structure_id:g.active_profile()?g.active_profile().structure_id:"none",profil_id:g.active_profile()?g.active_profile().type:"none",url:_(k).isNull()?b+d.current.url:k,parameters:_(h).isNull()?_(d.params).map(function(a,b){return b+"\x3d"+a}).join("\x26"):h})})}}]);
...
!1;a.selected(c);b.stopImmediatePropagation()};a.selected=function(b){b.selected=!b.selected;a.tiles_selected=0<_(a.available_tiles).select({selected:!0}).length};a.ok=function(){b.close(_(a.available_tiles).select({selected:!0}))};a.cancel=function(){b.dismiss()};p.query_applications().then(function(b){a.available_tiles=a.available_tiles.concat(_.chain(b).uniq(function(a){return a.application_id}).each(function(a){a.taxonomy="app";a.available=function(){return!_.chain(q).reject(function(a){return a.to_delete}).pluck("application_id").contains(a.application_id).value()}}).value());
