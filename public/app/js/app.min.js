...
function(){console.log("before");console.log(a.configure);t();console.log("after");console.log(a.configure);a.configure?(console.log("setting .configure to false"),a.configure=!1):(f.tree.tiles.forEach(function(b){b.configure=b===a?!b.configure:!1}),console.log("setting .configure to true"),a.configure=!0);console.log("after bis");console.log(a.configure)};a.update=function(b){void 0===b&&(b=void 0);var c={id:null,application_id:void 0};void 0!=b?(c.id=a.id,b.forEach(function(b){c[b]=a[b]})):c=angular.copy(a);
void 0!=c.application_id&&(c.application_id=""==c.application_id?null:c.application_id);null!=c.id&&n.update(c)};a.remove=function(){n.delete(a).$promise.then(function(a){r()})};!_(p[a.application_id]).isUndefined()&&_(p[a.application_id]).has("action")?a.action=p[a.application_id].action:a.action=function(){f.modification||("EXTERNAL"===a.type||_(a.application_id).isNull()||"PRONOTE"===a.application_id?g.log_and_open_link("PRONOTE"===a.application_id?"PRONOTE":"EXTERNAL",a.url):d.go("app",{appid:a.application_id}))};
!_(p[a.application_id]).isUndefined()&&_(p[a.application_id]).has("modify")&&(a=p[a.application_id].modify(a));return a},r=function(){k.tiles().then(function(a){a.forEach(function(a){a.taxonomy="app"});a=_(a).select(function(a){return(7!=moment().month()||a.summer||!_(f.user.profiles).isEmpty()&&!_(["ELV","TUT"]).includes(f.user.active_profile().type))&&(!f.user.profiles||!f.user.active_profile()||f.user.is_admin()||!_(a.hidden).includes(f.user.active_profile().type))&&("MAIL"===a.application_id?
