...
return a+1+" "+b[a]}),hour:function(a){10>a&&(a="0"+a);return a+":00 - "+a+":59"}};a.process_data=function(b){a.logs=_(b).map(function(a){a.timestamp=f(a.timestamp);a.weekday=a.timestamp.day();a.hour=a.timestamp.hour();return a});a.logs_SSO=_(a.logs).where({application_id:"SSO"});a.totals={users:_(a.logs).countBy(function(a){return a.user_id}),profile:_(a.logs).countBy(function(a){return a.profil_id})};a.logs=_(a.logs).reject(function(a){return"SSO"==a.application_id});a.log_structures=_.chain(a.logs).pluck("structure_id").uniq().map(function(b){return _(a.structures).findWhere({id:b})}).value();
