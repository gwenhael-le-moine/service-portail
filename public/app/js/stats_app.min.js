...
return b[a]}),hour:function(a){return a+"h"}};a.process_data=function(b){a.logs=_(b).map(function(a){a.timestamp=f(a.timestamp);a.weekday=a.timestamp.day();a.hour=a.timestamp.hour();return a});a.logs_SSO=_(a.logs).where({application_id:"SSO"});a.totals={users:_(a.logs).countBy(function(a){return a.user_id}),profile:_(a.logs).countBy(function(a){return a.profil_id})};a.logs=_(a.logs).reject(function(a){return"SSO"==a.application_id});a.log_structures=_.chain(a.logs).pluck("structure_id").uniq().map(function(b){return _(a.structures).findWhere({id:b})}).value();
a.log_applications=_.chain(a.logs).pluck("application_id").uniq().map(function(b){return _(a.applications).findWhere({id:b})}).value();a.log_profiles_types=_.chain(a.logs).pluck("profil_id").uniq().map(function(b){return _(a.profiles_types).findWhere({id:b})}).value();var h=["structure_id","application_id","profil_id","weekday","hour"],d=function(b,d){return[{key:"clicks",values:_.chain(d).keys().map(function(k){return{key:b,value:k,x:a.labels[b](k),y:d[k]}}).sortBy(function(a){switch(b){case "structure_id":case "profil_id":return-1*
a.y;default:return a.x}}).value()}]},c=function(a,b){return _.chain(b).map(function(b){return[b,d(b,_(a).countBy(b))]}).object().value()};a.stats={};a.stats.global=c(a.logs,h);a.stats.global.structure_id.push({key:"utilisateurs uniques",values:_.chain(a.logs).groupBy(function(a){return a.structure_id}).map(function(b,d){return{key:"utilisateurs uniques",x:a.labels.structure_id(d),y:_.chain(b).pluck("user_id").uniq().value().length}}).value()});a.stats.global.structure_id.push({key:"apps",values:_.chain(a.logs).groupBy(function(a){return a.structure_id}).map(function(b,
d){return{key:"apps",x:a.labels.structure_id(d),y:_.chain(b).pluck("application_id").uniq().value().length}}).value()});a.stats.global.profil_id.push({key:"utilisateurs uniques",values:_.chain(a.logs).groupBy(function(a){return a.profil_id}).map(function(b,d){return{key:"utilisateurs uniques",x:a.labels.profil_id(d),y:_.chain(b).pluck("user_id").uniq().value().length}}).value()});h.forEach(function(b){a.stats[b]=_.chain(a.stats.global[b][0].values).pluck("value").map(function(d){return[d,c(_(a.logs).select(function(a){return a[b]===
d}),_(h).difference([b]))]}).object().value()});_(a.stats.structure_id).each(function(b,d){b.profil_id.push({key:"utilisateurs uniques",values:_.chain(a.logs).where({structure_id:d}).groupBy(function(a){return a.profil_id}).map(function(b,d){return{key:"utilisateurs uniques",x:a.labels.profil_id(d),y:_.chain(b).pluck("user_id").uniq().value().length}}).value()})})};a.filter_data=function(b){return _(b).select(function(b){return(void 0==a.structures_types.selected||_.chain(a.structures).where({type:a.structures_types.selected.id}).pluck("id").contains(b.structure_id).value())&&
